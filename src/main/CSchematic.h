...
private:
    ...
    //std::map<std::string, FP>     pointerCastTable;     //Map of strings(DATA_TYPES) to their functions used for casting void* to other pointers
    std::map<std::type_index, FP>  pointerCastTable;
    std::string classFile;

public:
    CSchematic<T>(int killme);
...
template<class T>
CSchematic<T>::CSchematic(int killme){
